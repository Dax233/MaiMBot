...
from .sobriquet_db import SobriquetDB # MODIFIED
from .sobriquet_mapper import _build_mapping_prompt # MODIFIED
from .sobriquet_utils import select_sobriquets_for_prompt, format_sobriquet_prompt_injection # MODIFIED
...
logger = get_logger("SobriquetManager") # MODIFIED
...
class SobriquetManager: # MODIFIED
    """
    ...
    def __new__(cls, *args, **kwargs):
        if not cls._instance:
            with cls._lock:
                if not cls._instance:
                    logger.info("正在创建 SobriquetManager 单例实例...") # MODIFIED
                    cls._instance = super(SobriquetManager, cls).__new__(cls) # MODIFIED
                    cls._instance._initialized = False
        return cls._instance

    def __init__(self):
        """
        初始化 SobriquetManager。 # MODIFIED
        使用锁和标志确保实际初始化只执行一次。
        """
        ...
        with self._lock:
            ...
            logger.info("正在初始化 SobriquetManager 组件...") # MODIFIED
            self.is_enabled = global_config.group_sobriquet.enable_sobriquet_mapping # MODIFIED

            # 数据库处理器
            person_info_collection = getattr(db, "person_info", None)
            self.db_handler = SobriquetDB(person_info_collection) # MODIFIED
            if not self.db_handler.is_available():
                logger.error("数据库处理器初始化失败，SobriquetManager 功能受限。") # MODIFIED
            ...
            # 队列和线程
            self.queue_max_size = global_config.group_sobriquet.sobriquet_queue_max_size # MODIFIED
            # 使用 asyncio.Queue
            self.sobriquet_queue: asyncio.Queue = asyncio.Queue(maxsize=self.queue_max_size) # MODIFIED
            self._stop_event = threading.Event()  # stop_event 仍然使用 threading.Event，因为它是由另一个线程设置的
            self._sobriquet_thread: Optional[threading.Thread] = None # MODIFIED
            self.sleep_interval = global_config.group_sobriquet.sobriquet_process_sleep_interval  # 超时时间 # MODIFIED

            self._initialized = True
            logger.info("SobriquetManager 初始化完成。") # MODIFIED
    ...
    def start_processor(self):
        ...
        if not self.is_enabled:
            ...
            return
        if global_config.group_sobriquet.max_sobriquets_in_prompt == 0:  # 考虑有神秘的用户输入为0的可能性 # MODIFIED
            logger.error("[错误] 绰号注入数量不合适，绰号处理功能已禁用！")
            return

        if self._sobriquet_thread is None or not self._sobriquet_thread.is_alive(): # MODIFIED
            logger.info("正在启动绰号处理器线程...")
            self._stop_event.clear()
            self._sobriquet_thread = threading.Thread( # MODIFIED
                target=self._run_processor_in_thread,  # 线程目标函数不变
                daemon=True,
            )
            self._sobriquet_thread.start() # MODIFIED
            logger.info(f"绰号处理器线程已启动 (ID: {self._sobriquet_thread.ident})") # MODIFIED
        else:
            logger.warning("绰号处理器线程已在运行中。")

    def stop_processor(self):
        """停止后台处理线程。"""
        if self._sobriquet_thread and self._sobriquet_thread.is_alive(): # MODIFIED
            ...
            try:
                ...
                self._sobriquet_thread.join(timeout=10)  # 等待线程结束 # MODIFIED
                if self._sobriquet_thread.is_alive(): # MODIFIED
                    logger.warning("绰号处理器线程在超时后仍未停止。")
            except Exception as e:
                logger.error(f"停止绰号处理器线程时出错: {e}", exc_info=True)
            finally:
                if self._sobriquet_thread and not self._sobriquet_thread.is_alive(): # MODIFIED
                    logger.info("绰号处理器线程已成功停止。")
                self._sobriquet_thread = None # MODIFIED
        else:
    ...
    async def trigger_sobriquet_analysis( # MODIFIED
        self,
        ...
        if random.random() < global_config.group_sobriquet.sobriquet_analysis_probability: # MODIFIED
        ...
        try:
            # 1. 获取历史记录
            history_limit = global_config.group_sobriquet.sobriquet_analysis_history_limit # MODIFIED
            ...
            if user_ids_in_history:
                ...
                for user_id in user_ids_in_history:
                    if user_id in names_data:
                        ...
                        latest_sobriquet = next( # MODIFIED (variable name)
                            (
                                m["user_info"].get("user_nickname") # Assuming user_nickname means sobriquet here
                        ...
                        user_name_map[user_id] = (
                            latest_sobriquet or f"{global_config.bot.nickname}" # MODIFIED (variable name)
    ...
    async def get_sobriquet_prompt_injection(self, chat_stream: ChatStream, message_list_before_now: List[Dict]) -> str: # MODIFIED
        """
        ...
        try:
            ...
            all_sobriquets_data_with_uid = await relationship_manager.get_users_group_sobriquets( # MODIFIED
            ...
            if all_sobriquets_data_with_uid:
                ...
                selected_sobriquets_with_uid = select_sobriquets_for_prompt(all_sobriquets_data_with_uid) # 注意这里 # MODIFIED
                # selected_sobriquets_with_uid 的预期格式: List[Tuple[str, str, str, int]] -> (用户名, user_id, 绰号, 次数)
                injection_str = format_sobriquet_prompt_injection(selected_sobriquets_with_uid) # 注意这里 # MODIFIED
                if injection_str:
    ...
    async def _add_to_queue(self, item: tuple, platform: str, group_id: str):
        ...
        try:
            ...
            await self.sobriquet_queue.put(item) # MODIFIED
            logger.debug(
                f"已将项目添加到平台 '{platform}' 群组 '{group_id}' 的绰号队列。当前大小: {self.sobriquet_queue.qsize()}" # MODIFIED
            )
    ...
    async def _analyze_and_update_sobriquets(self, item: tuple): # MODIFIED
        ...
        if analysis_result.get("is_exist") and analysis_result.get("data"):
            sobriquet_map_to_update = analysis_result["data"] # MODIFIED
            logger.info(f"{log_prefix} LLM 找到绰号映射，准备更新数据库: {sobriquet_map_to_update}") # MODIFIED

            for user_id_str, sobriquet_val in sobriquet_map_to_update.items(): # MODIFIED (variable name)
                if not user_id_str or not sobriquet_val: # MODIFIED (variable name)
                    logger.warning(f"{log_prefix} 跳过无效条目: user_id='{user_id_str}', sobriquet='{sobriquet_val}'") # MODIFIED
                    continue
                ...
                try:
                    ...
                    self.db_handler.update_group_sobriquet_count(person_id, group_id, sobriquet_val) # MODIFIED
                except (OperationFailure, DuplicateKeyError) as db_err:
                    logger.exception(
                        f"{log_prefix} 数据库操作失败 ({type(db_err).__name__}): 用户 {user_id_str}, 绰号 {sobriquet_val}. 错误: {db_err}" # MODIFIED
                    )
                except Exception as e:
                    logger.exception(f"{log_prefix} 处理用户 {user_id_str} 的绰号 '{sobriquet_val}' 时发生意外错误：{e}") # MODIFIED
        else:
    ...
    def _filter_llm_results(self, original_data: Dict[str, str], user_name_map: Dict[str, str]) -> Dict[str, str]:
        ...
        for user_id, sobriquet_val in original_data.items(): # MODIFIED (variable name)
            ...
            if bot_qq_str and user_id == bot_qq_str:
                ...
                continue
            if not sobriquet_val or sobriquet_val.isspace(): # MODIFIED (variable name)
                logger.debug(f"过滤掉用户 {user_id} 的空绰号。")
                continue
            # person_name = user_name_map.get(user_id)
            # if person_name and person_name == sobriquet_val: # MODIFIED (variable name)
            #     logger.debug(f"过滤掉用户 {user_id} 的映射: 绰号 '{sobriquet_val}' 与其名称 '{person_name}' 相同。") # MODIFIED
            #     continue
            filtered_data[user_id] = sobriquet_val.strip() # MODIFIED (variable name)
    ...
    async def _processing_loop(self):
        ...
        while not self._stop_event.is_set():  # 仍然检查同步的停止事件
            try:
                ...
                item = await asyncio.wait_for(self.sobriquet_queue.get(), timeout=self.sleep_interval) # MODIFIED

                # 处理获取到的项目 (调用异步方法)
                await self._analyze_and_update_sobriquets(item) # MODIFIED

                self.sobriquet_queue.task_done()  # 标记任务完成 # MODIFIED
...
# 在模块级别创建单例实例
sobriquet_manager = SobriquetManager() # MODIFIED