...
class SobriquetDB:
            ...
            result = self.person_info_collection.update_one(
                {"person_id": person_id},
                {
                    "$setOnInsert": {
                        ...
                        "user_id": user_id_int,
                        "platform": platform,
                        "group_sobriquets": [],  # 初始化 group_sobriquets 数组
                    }
                },
                upsert=True,
            )
    ...
    def update_group_sobriquet_count(self, person_id: str, group_id_str: str, sobriquet: str):
        """
        ...
        try:
            ...
            result_inc = self.person_info_collection.update_one(
                ...
                {"$inc": {"group_sobriquets.$[group].sobriquets.$[nick].count": 1}},
                array_filters=[
            ...
            result_push_nick = self.person_info_collection.update_one(
                {
                    "person_id": person_id,
                    "group_sobriquets.group_id": group_id_str,  # 检查群组是否存在
                },
                {"$push": {"group_sobriquets.$[group].sobriquets": {"name": sobriquet, "count": 1}}},
            ...
            # 确保 group_sobriquets 数组存在 (作为保险措施)
            self.person_info_collection.update_one(
                {"person_id": person_id, "group_sobriquets": {"$exists": False}},
                {"$set": {"group_sobriquets": []}},
            )
            # 推送新的群组对象到 group_sobriquets 数组
            result_push_group = self.person_info_collection.update_one(
                {
                    "person_id": person_id,
                    "group_sobriquets.group_id": {"$ne": group_id_str},  # 确保该群组 ID 尚未存在
                },
                {
                    "$push": {
                        "group_sobriquets": {
