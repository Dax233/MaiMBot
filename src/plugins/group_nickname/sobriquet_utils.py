...
logger = get_logger("sobriquet_utils") # MODIFIED


def select_sobriquets_for_prompt( # MODIFIED
    all_sobriquets_info_with_uid: Dict[str, Dict[str, Any]] # 修改输入类型提示 # MODIFIED
) -> List[Tuple[str, str, str, int]]: # 修改返回类型提示 (用户名, user_id, 绰号, 次数)
    """
    从给定的绰号信息中，根据映射次数加权随机选择最多 N 个绰号用于 Prompt。

    Args:
        all_sobriquets_info_with_uid: 包含用户及其绰号和 UID 信息的字典，格式为 # MODIFIED
                        { "用户名1": {"user_id": "uid1", "sobriquets": [{"绰号A": 次数}, {"绰号B": 次数}]}, ... } # MODIFIED
    ...
    Returns:
        List[Tuple[str, str, str, int]]: 选中的绰号列表，每个元素为 (用户名, user_id, 绰号, 次数)。
                                    按次数降序排序。
    """
    if not all_sobriquets_info_with_uid: # MODIFIED
        return []
    ...
    for user_name, data in all_sobriquets_info_with_uid.items(): # MODIFIED
        ...
        sobriquets_list = data.get("sobriquets") # MODIFIED

        if not user_id or not isinstance(sobriquets_list, list): # MODIFIED
            logger.warning(f"用户 '{user_name}' 的数据格式无效或缺少 user_id/sobriquets。已跳过。 Data: {data}") # MODIFIED
            continue
            
        for sobriquet_entry in sobriquets_list: # MODIFIED
            if isinstance(sobriquet_entry, dict) and len(sobriquet_entry) == 1: # MODIFIED
                sobriquet, count = list(sobriquet_entry.items())[0] # MODIFIED
                if isinstance(count, int) and count > 0 and isinstance(sobriquet, str) and sobriquet: # MODIFIED
                    weight = count + smoothing_factor
                    candidates.append((user_name, user_id, sobriquet, count, weight)) # 添加 user_id # MODIFIED
                else:
                    logger.warning(
                        f"用户 '{user_name}' (UID: {user_id}) 的绰号条目无效: {sobriquet_entry} (次数非正整数或绰号为空)。已跳过。" # MODIFIED
                    )
            else:
                logger.warning(f"用户 '{user_name}' (UID: {user_id}) 的绰号条目格式无效: {sobriquet_entry}。已跳过。") # MODIFIED
    ...
    try:
        ...
        if len(selected_candidates_with_weight) < num_to_select:
            ...
            selected_ids = set(
                (c[0], c[1], c[2]) for c in selected_candidates_with_weight # (user_name, user_id, sobriquet) # MODIFIED
            )
...
def format_sobriquet_prompt_injection(selected_sobriquets_with_uid: List[Tuple[str, str, str, int]]) -> str: # 修改输入类型 # MODIFIED
    """
    将选中的绰号信息（含UID）格式化为注入 Prompt 的字符串。

    Args:
        selected_sobriquets_with_uid: 选中的绰号列表 (用户名, user_id, 绰号, 次数)。 # MODIFIED

    Returns:
        str: 格式化后的字符串，如果列表为空则返回空字符串。
    """
    if not selected_sobriquets_with_uid: # MODIFIED
        return ""
    ...
    for user_name, user_id, sobriquet, _count in selected_sobriquets_with_uid: # 解包时加入 user_id # MODIFIED
        user_key = (user_name, user_id) # 使用 (user_name, user_id) 作为键
        if user_key not in grouped_by_user:
            grouped_by_user[user_key] = []
        grouped_by_user[user_key].append(f"“{sobriquet}”") # MODIFIED

    for (user_name, user_id), sobriquets_list_str in grouped_by_user.items(): # MODIFIED (var name)
        sobriquets_str = "、".join(sobriquets_list_str) # MODIFIED (var name)
        # 格式化输出，例如: "- 张三(12345)，ta 可能被称为：“三儿”、“张哥”"
        prompt_lines.append(f"- {user_name}({user_id})，ta 可能被称为：{sobriquets_str}") # MODIFIED (var name)
