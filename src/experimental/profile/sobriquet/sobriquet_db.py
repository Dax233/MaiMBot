...
class SobriquetDB:
    ...
    def update_sobriquet_count(self, person_id: str, group_id_str: str, sobriquet: str):
        """
        ...
        """
        if not self.is_available():
            logger.error("数据库集合不可用，无法执行 update_sobriquet_count。")
            return
        ...
        try:
            ...
            result_inc = self.person_info_collection.update_one(
                {
                    "person_id": person_id,
                    "sobriquets": {"$elemMatch": {"group_id": group_id_str, "sobriquets.name": sobriquet}},
                },
                {"$inc": {"sobriquets.$[group].sobriquets.$[nick].count": 1}},
                array_filters=[
            ...
            if result_inc.modified_count > 0:
                # logger.debug(f"成功增加 person_id {person_id} 在群组 {group_id_str} 中绰号 '{sobriquet}' 的计数。")
                return  # 成功增加计数，操作完成
            ...
            result_push_nick = self.person_info_collection.update_one(
                {
                    "person_id": person_id,
                    "sobriquets.group_id": group_id_str,  # 检查群组是否存在
                },
                {"$push": {"sobriquets.$[group].sobriquets": {"name": sobriquet, "count": 1}}},
            ...
            # 确保 sobriquets 数组存在 (作为保险措施)
            self.person_info_collection.update_one(
                {"person_id": person_id, "sobriquets": {"$exists": False}},
                {"$set": {"sobriquets": []}},
            )
            # 推送新的群组对象到 sobriquets 数组
            result_push_group = self.person_info_collection.update_one(
                {
                    "person_id": person_id,
                    "sobriquets.group_id": {"$ne": group_id_str},  # 确保该群组 ID 尚未存在
                },
                {
                    "$push": {
                        "sobriquets": {
                            "group_id": group_id_str,
                            "sobriquets": [{"name": sobriquet, "count": 1}],
                        }
                    }
                },
            )
